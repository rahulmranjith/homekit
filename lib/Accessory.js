...
Accessory.prototype.addService = function(service) {
    ...
    if (!this.bridged) {
        ...
        this.emit('service-configurationChange', clone({
            accessory: this,
            service: service
        }));
    }
    ...
    service.on('characteristic-change', function(change) {
        this.emit('service-characteristic-change', clone(change, {
            service: service
        }));

        // if we're not bridged, when we'll want to process this event through our HAPServer
        if (!this.bridged)
            this._handleCharacteristicChange(clone(change, {
                accessory: this,
                service: service
            }));
...
Accessory.prototype._handleSetCharacteristics = function(data, events, callback, remote, connectionID) {
    ...
    data.forEach(function(characteristicData) {
        ...
        if (typeof ev !== 'undefined') {
            ...
            // the request.
            var eventName = aid + '.' + iid;
            if (characteristic.displayName == "Motion Detected" && ev === false) {
                debug('[%s] **** Ignoring Unresigster Request for "Motion Detected"', this.displayName); // set ev to undefined
                ev = true;// This change has been done as per the bug reported. The notifications over apple tv for 
                //motion detector are getting unregistered after registering the set. The original fix was to set the ev
                // to true based on aid and iid . But that caused issues for me as the aid kept changing .So this fix was made based on the characteristic displayName
            }
