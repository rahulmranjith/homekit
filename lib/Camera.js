...
Camera.prototype.handleSnapshotRequest = function(request, callback) {
    ...
    // var raspistill = `raspistill -w ${request.width} -h ${request.height} -t 5 -o ` + __dirname + `/res/snapshotnew.jpg`;
    //
    // shell.exec(raspistill, function(code, stdout, stderr) {
    //     var snapshot = undefined;
    //     console.log("directory name " + __dirname);
    //     if (code === 0) {
    //         snapshot = fs.readFileSync(__dirname + '/res/snapshotnew.jpg');
    //     }
    ...
    if (sessionID) {
        ...
        if (requestType == "start") {
            ...
            if (sessionInfo) {
                ...
                console.log(targetAddress + targetVideoPort + videoKey);
                //original let ffmpegCommand = '-re -f avfoundation -r 29.970000 -i 0:0 -threads 0 -vcodec libx264 -an -pix_fmt yuv420p -r '+ fps +' -f rawvideo -tune zerolatency -vf scale='+ width +':'+ height +' -b:v '+ bitrate +'k -bufsize '+ bitrate +'k -payload_type 99 -ssrc 1 -f rtp -srtp_out_suite AES_CM_128_HMAC_SHA1_80 -srtp_out_params '+videoKey.toString('base64')+' srtp://'+targetAddress+':'+targetVideoPort+'?rtcpport='+targetVideoPort+'&localrtcpport='+targetVideoPort+'&pkt_size=1378';

                //gituhub issue compiled let ffmpegCommand = '-re -i rtsp://127.0.0.1:8554/pi_encode.h264 -threads 0 -vcodec h264_omx -an -pix_fmt yuv420p -r ' + fps + ' -f rawvideo -tune zerolatency -vf scale=' + width + ':' + height + ' -b:v ' + bitrate + 'k -bufsize ' + bitrate + 'k -payload_type 99 -ssrc 1 -f rtp -srtp_out_suite AES_CM_128_HMAC_SHA1_80 -srtp_out_params ' + videoKey.toString('base64') + ' srtp://' + targetAddress + ':' + targetVideoPort + '?rtcpport=' + targetVideoPort + '&localrtcpport=' + targetVideoPort + '&pkt_size=1378';
