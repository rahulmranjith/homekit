...
HAPServer.prototype._handleIdentify = function(request, response, session, events, requestData) {
    ...
    if (!this.allowInsecureRequest && this.accessoryInfo.paired()) {
        ...
        response.end(JSON.stringify({
            status: HAPServer.Status.INSUFFICIENT_PRIVILEGES
        }));
...
HAPServer.prototype._handleCharacteristics = function(request, response, session, events, requestData) {
    ...
    if (request.method == "GET") {
        ...
        if (requestData.length == 0) {
            ...
            response.end(JSON.stringify({
                status: HAPServer.Status.INVALID_VALUE_IN_REQUEST
            }));
        ...
        // JSP - debugging
        //RMR
        // debug("jspdebug data =", data);
        // debug("jspdebug this =", this);
        //
        // if (data[0].aid === 1 && data[0].iid === 8 && data[0].ev == false) {
        //     debug(">>>>> OVERRIDIDING <<<<<<<<<");
        //     data[0].ev = true;
        // }
...
function HAPEncryption() {
    ...
    this.accessoryToControllerCount = {
        value: 0
    };
    this.controllerToAccessoryCount = {
        value: 0
    };
